...
type Pass = { pts: Pt[]; cum: number[]; width: number[] };
type Prepared = { passes: Pass[]; length: number };
...
function prepare(base: Pt[], style: LineStyle): Prepared {
...
function grain(colour: string, seed: number, density: number): HTMLCanvasElement {
  const size = 64;
