 ...
 *
 * A landscape watercolour sketchbook lies on the sheet, bound down its left
 * side: boards in burnt-sienna book cloth, a paper label with "notebook"
 * written on it, an elastic band. Its pages are heavy white cold-pressed
 * paper with a tooth and no ruling. It is sketched and painted as the
...
export class Book {
  ...
  resize() {
    ...
    this.ph = this.portrait ? Math.min(this.H * 0.5, (this.W * 0.9) / ASPECT) : Math.min(this.H * 0.72, (this.W * 0.94) / 2 / ASPECT);
  ...
  /** Heavy white cold-pressed paper: its tooth, in light and shade. */
  ...
  private wrap(f: Face, s: string, x: number, y: number, width: number, size: number, lead: number) {
    ...
    for (const w of words) {
      ...
      if (ctx.measureText(test).width > width * this.pw && line) {
        this.text(f, line, x, yy, size, { weight: 500, colour: 'rgba(29,29,33,0.75)' });
        line = w;
        yy += lead;
      } else line = test;
    }
    if (line) this.text(f, line, x, yy, size, { weight: 500, colour: 'rgba(29,29,33,0.75)' });
  }
  ...
  private coverFace(): Face {
    ...
    ctx.fillStyle = '#8f4326';
    ctx.fillRect(0, 0, w, h);
    this.washOn(f, (paint) => {
      paint([[0, 0], [w, 0], [w, h], [0, h]], BOARD, 0.12, 14, 0.04);
      paint([[0, 0], [w * 0.07, 0], [w * 0.07, h], [0, h]], '#6b2f19', 0.14, 10, 0.04);
      for (let k = 0; k < 8; k++) paint(blob(r() * w, r() * h, w * 0.3, h * 0.12, r), '#a9573a', 0.05, 6, 0.3);
    }, 12);
    // The label.
    const lx = w * 0.2;
    const ly = h * 0.26;
    ...
    ctx.fillRect(w * 0.9, 0, w * 0.022, h);
    // Worn corners.
